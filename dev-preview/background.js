(function () {
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    // Return the browser window width (reliable, works even when outerWidth=0)
    if (msg.type === "MRBB_GET_WINDOW_WIDTH") {
      // MUST use sender's windowId, not getCurrent()
      // getCurrent() might return a different window (e.g. MCP extension's window)
      var windowId = sender.tab ? sender.tab.windowId : undefined;
      if (windowId) {
        chrome.windows.get(windowId, function (win) {
          sendResponse({ width: win.width || 0 });
        });
      } else {
